(function($) {
  ...
  function stepTextShadow(fx) {
    ...
    if (fx.end.begin || (fx.state < 1)) {
      $(fx.elem).css('box-shadow', fx.end.color + ' 0 0 ' + updatedRadius + 'px');
    } else {
      $(fx.elem).css('box-shadow', $(fx.elem).data('glow.originalGlow'));
    }
  }
  ...
  function addGlow(opts) {
    ...
    with($(this)) {
      ...
      data('glow.originalGlow', (css('box-shadow') || 'none'));
    }
